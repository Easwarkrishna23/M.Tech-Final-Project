"""
Standalone script: re-run Nettack + defense only, then patch the
phase45 cache in-place.

Usage:
    python3 rerun_nettack.py
"""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print(f"  Re-running Nettack only "
          f"(targets={cfg.attack.nettack_target_count}, "
          f"perturbs={cfg.attack.nettack_n_perturbations})")
    print("=" * 60)
    ...
    # ── Run Nettack ───────────────────────────────────────────────────────────
    print(f"\n  Running nettack ...")
    attack_result = nettack(
        graph=cora,
        model=model,
        params=params,
        n_perturbations=cfg.attack.nettack_n_perturbations,
        target_count=cfg.attack.nettack_target_count,
        direct_attack=cfg.attack.nettack_direct,
    )
    ...
    for dname, single in [("gnnguard", dr.gnnguard), ("ontology", dr.ontology)]:
        ...
        np.savez(def_dir / f"defended_nettack_{dname}.npz",
    ...
    cache["attack_accs"]["nettack"]      = float(atk_acc)
    cache["defended_accs"]["nettack"]    = float(best_acc)
    cache["attack_metrics"]["nettack"]   = {k: float(v) for k, v in atk_m.items()}
